          ...
          <div class="card-body" style="padding: 10px;height: auto;align-items: center;">
            <form method="post">
                <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="no_surat" value="<?= $mutasi['no_surat'] ?>" name="no_surat">
                  <?= form_error('no_surat','<small class="text-danger text-error">','</small>'); ?>
                </div>
                <div class="form-group">
                  <label for="nama">Nama</label>
                  <input type="text" class="form-control" id="nama" value="<?= $mutasi['nama'] ?>" name="nama">
                  <?= form_error('nama','<small class="text-danger text-error">','</small>'); ?>
                </div>
                <div class="form-group">
                  <label for="tanggal_masuk">Tanggal Masuk</label>
                  <input type="date" class="form-control" id="tanggal_masuk" name="tanggal_masuk" value="<?= date('Y-m-d',strtotime($mutasi['tanggal_masuk'])); ?>">
                  <?= form_error('tanggal_masuk','<small class="text-danger text-error">','</small>'); ?>
                </div>
                <div class="form-group">
                  <label for="perihal">Perihal</label>
                    <textarea class="form-control" id="perihal" name="perihal"><?= $mutasi['perihal'] ?></textarea>
